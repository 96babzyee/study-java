...
public class MemberDao {
	...
	public int insert(Member member) {
		int count = 0; // member테이블에 sql문 변경된 행의 개수
		...
		try {
			...
			pstmt.setString(2, member.getName()); //물음표 2번자리에 String형으로 설정 - 테이블 varchar형 컬럼과 호환
			//sql문 실행
			count = pstmt.executeUpdate(); //insert문 DB서버로 전송하여 실행
	...
	public int deleteAll() {
		...
		try {
			...
			sql="DELETE FROM member"; // 테이블 내용만 전부 삭제
			
			pstmt = con.prepareStatement(sql);
			//sql문 실행
			count = pstmt.executeUpdate(); // delete문 DB서버로 전송하여 실행
